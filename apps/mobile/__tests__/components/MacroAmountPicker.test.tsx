...
describe('MacroAmountPicker', () => {
  ...
  describe('increment/decrement', () => {
    it('calls onChange with incremented value when +1 is pressed', () => {
    ...
    it('calls onChange with decremented value when -1 is pressed', () => {
      const onChange = jest.fn();
      const { getByText } = render(
        <MacroAmountPicker {...defaultProps} onChange={onChange} value={25} />
      );
  ...
  describe('quick select buttons', () => {
    it('displays default quick select buttons', () => {
      const { getByText } = render(
  ...
  describe('last saved value', () => {
    it('shows last saved value button when different from quick values', () => {
      const { getByText } = render(
        <MacroAmountPicker {...defaultProps} min={0} max={100} lastSavedValue={33} />
      );
    ...
    it('does not show last saved value if it matches a quick value', () => {
      ...
      // Should only have one "20" (the quick value button, not a duplicate)
