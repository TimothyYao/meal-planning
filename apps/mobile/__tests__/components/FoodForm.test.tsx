...
jest.mock('../../storage', () => ({
  ...
  generateFoodId: jest.fn(() => Promise.resolve('new-food-id-abc123')),
}));
...
describe('FoodForm', () => {
  ...
  describe('calorie calculation', () => {
    it('calculates calories automatically from macros', () => {
      const { getAllByText, getByText } = render(<FoodForm {...defaultProps} />);

      // Default macros are 0, so calories should show 0
